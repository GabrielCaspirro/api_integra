...
function GetAllEstudantes(res){
  ...
  function InserirEstudante(req, res) {
    const { rm, nome, curso, modulo_ano, email, telefone, id_instituicao } = req.body;

    if (!rm || !nome || !email || !id_instituicao) {
      return res.status(400).json({ erro: 'Campos obrigatórios faltando.' });
    }

    const senhaGerada = (Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000).toString();
    console.log('Senha gerada:', senhaGerada);

    const sql = `
      INSERT INTO aluno (nome, email, senha, telefone, data_cadastro, curso, rm, modulo_ano, id_instituicao)
      VALUES (?, ?, ?, ?, current_timestamp(), ?, ?, ?, ?)
    `;

    bcrypt.hash(senhaGerada, 10, (err, hash) => {
      if (err) return res.status(500).json({ erro: 'Erro ao criptografar senha.' });

      conexao.query(
        sql,
        [nome, email, hash, telefone, curso, rm, modulo_ano, id_instituicao],
        (err, resultado) => {
          if (err) {
          ...
          try {
            emailController.EnviarEmail(email, senhaGerada);
          } catch (e) {
